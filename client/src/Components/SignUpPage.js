...
const SignUpPage = () => {
    ...
    const passwordConfermationInput = (e) => {
    ...
    const formSubmit = (e) => {
        ...
        api().get('sanctum/csrf-cookie').then(() => { // Authenticate the request via Sanctum cookies for csrf...
            api().post('register', { // Post the register form with name, email, password and password_confirmation...
                name: name,
                email: email,
                password: password,
                ...
                //TODO 1) Make a better error handling,
            ));
        });
    }
    return (
        <div className="bg-indigo-50">
            <div className="container mx-auto bg-gray-50 min-h-screen border shadow-md flex flex-col justify-center items-center">
                <div className="w-full max-w-xl md:w-2/3 bg-white rounded-lg shadow-lg flex flex-col justify-center items-start">
                    <h1 className="mt-8 mx-auto text-2xl text-blue-500">SignUp Form</h1>
                    <form className="p-10 w-full">
                        <label htmlFor="name" className="my-4 text-xs sm:text-sm tracking-wide text-gray-700 block">name:</label>
                        <input
                            type="text"
                            id="name"
                        ...
                        <input
                            type="password"
                            id="password"
                            className="w-full block text-sm sm:text-base placeholder-gray-500 pl-4 pr-4 rounded-lg border border-gray-400 py-2 focus:outline-none focus:border-blue-500"
                            placeholder="Enter Password..."
                            onChange={passwordInput}
                            required />
                        <label htmlFor="password_confirmation" className="my-4 text-xs sm:text-sm tracking-wide text-gray-700 block">Password Confirmation:</label>
                        <input
                            type="password"
                            ...
                            onChange={passwordConfermationInput}
                            required />
